...
public:
	...
	void tank_tick()
	{
		...
		{
			clock_t cur = clock();
			float dTime = (float)(cur - nt_reborn_t) / CLOCKS_PER_SEC;
			if (dTime > m_fRebornNT)
			{
				nt_reborn_t = cur;
				SetAlive(true);
	...
	void Move(ETankDir dir)
	{
		...
		if (m_bIsAI && found && m_MoveCounter < 4)
		{
	...
	void ProcessKeyBoard(char ch)
	{
		...
		case 'f':
		case 'F':
			if (this == GameMode::instance().tank01)
				fire();
			break;
		case 'h':
		case 'H':
			if (this == GameMode::instance().tank02)
				fire();
			break;
...
public:
	...
	float m_fRebornNT = 4.0f;


	int m_iCounterKill = 0;
	int m_iCounterDead = 0;
