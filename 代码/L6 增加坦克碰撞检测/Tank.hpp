...
public:
	...
	bool judge(int offset, bool if_col)
	{
		for (int i = 0; i < 3; i++)
		{
			int judge_op = 0;
			if(if_col)
				judge_op = GameMode::instance().m_pmap->map[(pos_x + offset)* Common::LEN + pos_y + i];
			else
				judge_op = GameMode::instance().m_pmap->map[(pos_x + i)* Common::LEN + pos_y + offset];
