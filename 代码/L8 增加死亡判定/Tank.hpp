...
public:
	...
	void clear()
	{
		for (int i = 0; i < 3; i++)
		{
			...
			{
				if (GameMode::instance().m_pmap->map[(i + pos_x)*Common::LEN + pos_y + j] == Common::GRASS ||
					GameMode::instance().m_pmap->map[(i + pos_x)*Common::LEN + pos_y + j] == Common::BULLET)
					continue;
	...
	bool judge(int x, int y, bool if_col) {
		{
			for (int i = 0; i < 3; i++)
			{
				...
				if (judge_op == Common::STONE || judge_op == Common::WALL || judge_op == Common::WATER || judge_op == Common::TANK || judge_op == Common::BULLET)
				{
					return false;
				}
			}
